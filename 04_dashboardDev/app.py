...
classify = pd.read_csv(f"classification.csv",index_col=0)
...
#             html.P("Select any of the load profiles and preset scenarios of home-work commuting activities in Manhattan."),
#         ],
#     )
...
#                 labelStyle={'display': 'inline', 'margin-right': 10}
#             ),
#             html.P("3. With offshore wind projects"),
#             # html.P("3. With offshore wind projects (current & planned)"),
#             html.H2("Commuting Scenario"),
#             html.P("1. Select Transit Pattern"),
#             dcc.RadioItems(
#                 id='transit_pattern',
#                 className="dcc_control",
#                 options=[
...
#     if transit_pattern==pattern_list[0]:
#         comm_df = commuter_model_0[['Reassigned','PERWT','lat','lon']].copy()
#     else:
#         commuter_model_of_choice_idx = transit_pattern + "+" + wfh_level
#         comm_df = available_commuter_models[commuter_model_of_choice_idx][['Reassigned','PERWT','lat','lon']].copy()

#     all_CategoricalDtype = ['Walk','Escooter','Bicycle','Ferry','WFH','Autos','Bus','Motorcycle','Taxicab','CommuterRail','Subway','Other']
#     specific_CategoricalDtype = comm_df['Reassigned'].unique().tolist()
#     final_CategoricalDtype = sorted(set(all_CategoricalDtype) & set(specific_CategoricalDtype), key = all_CategoricalDtype.index)

#     mode_order = CategoricalDtype(final_CategoricalDtype,ordered=True)
...
#     if transit_pattern==pattern_list[0]:
#         com_df = commuter_model_0[['TransMode','PERWT','ARRIVES_AT_WORK_HOUR','LEAVE_WORK_HOUR']].copy()
#     else:
#         commuter_model_of_choice_idx = transit_pattern + "+" + wfh_level
#         com_df = available_commuter_models[commuter_model_of_choice_idx][['TransMode','PERWT','ARRIVES_AT_WORK_HOUR','LEAVE_WORK_HOUR']].copy()
...
#     flag_columns = ['FLAG_AUTO','FLAG_EBIKE','FLAG_EBUSES','FLAG_COMMUTERRAIL','FLAG_FERRY','FLAG_MOTORCYCLE','FLAG_SUBWAY','FLAG_TAXICAB','FLAG_WFH','FLAG_WALK','FLAG_ESCOOTER','Reassigned','PERWT']

#     if transit_pattern==pattern_list[0]:
#         com_df = commuter_model_0[flag_columns].copy()
#     else:
#         commuter_model_of_choice_idx = transit_pattern + "+" + wfh_level
#         com_df = available_commuter_models[commuter_model_of_choice_idx][flag_columns].copy()
...
#     if transit_pattern==pattern_list[0]:
#         com_df = commuter_model_0[['Reassigned','Reassigned_Parent','PERWT']].copy()
#     else:
#         commuter_model_of_choice_idx = transit_pattern + "+" + wfh_level
#         com_df = available_commuter_models[commuter_model_of_choice_idx][['Reassigned','Reassigned_Parent','PERWT']].copy()
...
#     if transit_pattern==pattern_list[0]:
#         com_df = commuter_model_0[['Subregion','Reassigned','PERWT']].copy()
#     else:
#         commuter_model_of_choice_idx = transit_pattern + "+" + wfh_level
#         com_df = available_commuter_models[commuter_model_of_choice_idx][['Subregion','Reassigned','PERWT']].copy()
